...
class CachedCollection {
    constructor(collection, cache, options) {
        ...
        this.cachePrefix = `mongo-${this.collection.collectionName
            ? this.collection.collectionName
            : 'test'}-`;
        this.allCacheKeys = `${this.cachePrefix}all-keys`;
        this.loader = new dataloader_1.default(ids => this.collection
            .find({ _id: { $in: ids.map(id => new mongodb_1.ObjectID(id)) } })
            .toArray()
            .then(docs => CachedCollection.remapDocs(docs, ids)));
        this.queryLoader = new dataloader_1.default(queries => this.collection
            .find({ $or: queries })
            .toArray()
            .then(items => queries.map(query => items.filter(sift_1.default(query)))));
    }
