...
class MongoDataSource extends apollo_datasource_1.DataSource {
  ...
  initialize(config = {}) {
    ...
    const cache =
      config.cache || new apollo_server_caching_1.InMemoryLRUCache();
    this.collections = this.rawCollections.reduce(
      (acc, collection) =>
        Object.assign(Object.assign({}, acc), {
          [collection.collectionName]: new cache_1.CachedCollection(
            collection,
            cache,
            {
